...
export const getLeaderboard = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id, stage_name, avatar_url, city, total_score, performances_count, best_score")
...
export const updateMyProfile = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { stage_name?: string; city?: string; bio?: string; avatar_url?: string }) => input)
  .handler(async ({ data, context }) => {
    const patch: { stage_name?: string; city?: string; bio?: string; avatar_url?: string } = {};
